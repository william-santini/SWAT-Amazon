...
<pre lang="markdown"> ``` your_project/ │ ├── txtInOut/ │ ├── SWAT-Amazon.exe │ ├── Qss_forcing.PRN │ ├── Qsf_forcing.PRN │ ├── hdwnstrm.TXT │ └── ... (other standard SWAT2012 input/output files) │ ├── SWAT-Amazon-Calib.Rmd ├── tools_and_functions_global.R ├── setpar_test.R ├── setpar_bestcal.R ├── setpar_paral_tibble.R └── setpar_sen.R ``` </pre>
